		...
		try {
			...
			if (content.size != 0) {
				...
				while( getline(fdescStream, line) ) {
					...
					for(int i = 1; i < nbFiles; i++, baseIndex++) {
						if( baseIndex >= content.size ) {
							std::cout << "Line " << line <<"nbFIles " << nbFiles<< " baseIndex " << baseIndex << " content size " <<  content.size << std::endl ;
							throw UserException(ERRCODE_INVALID_PARAM, "DAGDA Exception: uncompleted list of transfered files");
						}
						dagda_get_file(content.elt_ids[baseIndex], &fName);
