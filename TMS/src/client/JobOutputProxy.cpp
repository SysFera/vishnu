...
JobOutputProxy::getJobOutPut(const std::string& jobId) {
	...
	if(!diet_call(getJobOutPutProfile)) {
		...
		try {
			...
			if (content.size != 0) {
				if( ! moutDir.size() ) {
					moutDir = (bfs::path(bfs::current_path().string())).string() + "/OUPUT_" + jobId;
				...
				ListStrings fDescVec ;
				boost::split(fDescVec, fDescStr, boost::is_any_of(" ")) ;

				if(content.size - 1 != fDescVec.size()) {
				...
				for(int i = 1; i < content.size; i++) {
					...
					vishnu::boostMoveFile(std::string(fName), moutDir, fDescVec[i-1]);
					if(fName) free(fName) ;
				}
			}
