...
bool isStartByQuote(const std::string& str, const char quote) {
  return (str.size() >= 1 && *(str.begin())==quote);
}

bool cleanString(std::string& str, const char& quote) {
...
getStreamTokens(const std::string& str) {
  ...
  stream_str.str(str);
  std::copy(std::istream_iterator<std::string>(stream_str),
            std::istream_iterator<std::string>(),
            std::back_inserter<std::vector<std::string> >(streamTokens));
