...
public:
  ...
  testFromString(){
    ...
    BOOST_CHECK(s.getName().compare(mname) == 0 &&
  ...
  testToString(){
    ...
    std::string res = mname+"$$$"+muri+"$$$"+mservices.at(0)+"$$$"+mservices.at(1);
    BOOST_CHECK(res.compare(s.toString()) == 0);
}
  ...
  testAdd(){
    ...
    Server s(mname, mservices, muri);
    s.add(str);
    BOOST_CHECK(s.getServices().size() == 3 &&
                s.hasService(str));
  }
  ...
  testRemove(){
    ...
    Server s(mname, mservices, muri);
    s.remove(rem);
    BOOST_CHECK(s.getServices().size() == 1 &&
                !s.hasService(rem));
  }
  ...
  testRemoveInvalid(){
    ...
    BOOST_CHECK(res==size);
  }

  void
  testHasService(){
    Server s(mname, mservices, muri);
    BOOST_CHECK(s.hasService(mservices.at(0)) &&
  ...
  testHasServiceBad(){
    Server s(mname, mservices, muri);
    BOOST_CHECK(!s.hasService("bad"));
  }

  void
  testGetName(){
    Server s(mname, mservices, muri);
    BOOST_CHECK(s.getURI().compare(muri)==0);
  }
  void
  testGetServices(){
    Server s(mname, mservices, muri);
    BOOST_CHECK(s.getServices().at(0).compare(mservices.at(0))==0 &&
