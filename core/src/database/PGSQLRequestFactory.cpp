...
PGSQLRequestFactory::PGSQLRequestFactory():RequestFactory(){
  ...
  mrequest.insert (std::pair<int, std::string>(VR_UPDATE_AUTHACCOUNT_WITH_AUTHSYSTEM, "update authaccount set status=%1% from authsystem where authsystem.numauthsystemid=authaccount.authsystem_authsystemid and authsystem.authsystemid='%2%';"));
}
