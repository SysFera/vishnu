...
UserException::initMsg(){
  ...
  mp.insert(std::pair<int, std::string>(ERRCODE_CLI_ERROR_NO_SESSION, std::string("There is no open session in this terminal")));
  mp.insert(std::pair<int, std::string>(ERRCODE_CLI_ERROR_MISSING_PARAMETER, std::string("Missing parameters")));
  mp.insert(std::pair<int, std::string>(ERRCODE_CLI_ERROR_DIET, std::string("Vishnuinitialization failed")));
