...
diet_profile_desc_t* getChangeOwnerProfile() {
  diet_profile_desc_t* result = diet_profile_desc_alloc("FileChangeOwner", 4, 4, 5);
...
int solveChangeOwner (diet_profile_t* profile) {
  string localPath, localUser,userKey="", acLogin, machineName;
  char* path, *user, *host,*sessionKey, *owner, *errMsg = NULL;
  std::string finishError ="";
  int mapperkey;
  std::string cmd = "";
  ...
  try {
    ...
    //MAPPER CREATION
    Mapper *mapper = MapperRegistry::getInstance()->getMapper(FMSMAPPERNAME);
    mapperkey = mapper->code("vishnu_ch_own");
    mapper->code(owner, mapperkey);
    mapper->code(std::string(host)+":"+std::string(path), mapperkey);
    cmd = mapper->finalize(mapperkey);
