...
BOOST_AUTO_TEST_CASE(ChangeGroup_Base)
{
  ...
  try {
    ...
    BOOST_REQUIRE( chgrp(sessionKey, m_test_fms_user_local_group, fileFullPath1) == 0);
    // To check the success
    FileStat stat;
    BOOST_REQUIRE( vishnu::stat(sessionKey, fileFullPath1, stat) == 0);
    BOOST_CHECK( stat.getGroup() == m_test_fms_user_local_group);
    // Cleanup
...
BOOST_AUTO_TEST_CASE(ChangeGroup_Exceptions)
{
  ...
  try {
    ...
    BOOST_CHECK_THROW( chgrp(sessionKey, m_test_fms_user_local_group, invalidFullPath), VishnuException);
    // E2 case
    string noAccessLocalPath = "/root/abc";
    string noAccessFullPath = m_test_fms_host1 + sep + noAccessLocalPath;
    BOOST_CHECK_THROW( chgrp(sessionKey, m_test_fms_user_local_group, noAccessFullPath), VishnuException);
    // E3 case
    string invalidMachineId = "tt";
    string invalidMachineFullPath = invalidMachineId + sep + m_test_fms_dir1;
    BOOST_CHECK_THROW( chgrp(sessionKey, m_test_fms_user_local_group, invalidMachineFullPath), VishnuException);
