...
void stress(int cpt,const string& sessionKey,int type){
...
void forker(int cpt,const string& sessionKey, int type){
  cpt--;

  switch(fork()){
    case 0 :
      stress( cpt,sessionKey,type) ;
      break;
    case -1 :
...
int main(int argc, char** argv){
  ...
  try{
    ...
    forker(cpt, sess.getSessionKey(),type);
  }
